...
export default function MarketPhotoManager({ marketId, photos, uploadAction, deleteAction, reorderAction }: Props) {
  ...
  return <section className="border border-black/10 bg-white/60 p-5">
    ...
    {orderedPhotos.length ? <form action={reorderAction} className="mt-5"><input name="market_id" type="hidden" value={marketId} /><input name="photo_ids" type="hidden" value={JSON.stringify(orderedPhotos.map((photo) => photo.id))} /><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{orderedPhotos.map((photo) => <div draggable key={photo.id} onDragStart={() => setDraggedId(photo.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => movePhoto(photo.id)} className="border border-black/10 bg-[#F2F0EA] p-2"><img src={photo.photo_url} alt="" className="aspect-square w-full object-cover" /><div className="mt-2 flex items-center justify-between gap-2"><span className="text-xs text-black/50">Drag to move</span><button className="text-xs text-red-700" formAction={deleteAction} name="id" value={photo.id} type="submit">Delete</button></div></div>)}</div><button className="mt-4 border border-black/20 px-3 py-2 text-sm" type="submit">Save photo order</button></form> : <p className="mt-5 border border-dashed border-black/20 px-4 py-6 text-center text-sm text-black/50">No photos uploaded yet.</p>}
  </section>;
}
